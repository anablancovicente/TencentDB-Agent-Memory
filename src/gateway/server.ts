...
export class TdaiGateway {
  ...
  private async handleRecall(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    const result = await this.core.handleBeforeRecall(body.query, body.session_key, topK, body.user_id);
        ...
        try {
          const response = await fetch(`${llmCfg.baseUrl}/chat/completions`, {
            ...
            body: JSON.stringify({
              ...
              // Reasoning models (deepseek-v4-flash via cliproxy): thinking must be
              // disabled or reasoning tokens eat the max_tokens budget (mirrors llm-runner).
